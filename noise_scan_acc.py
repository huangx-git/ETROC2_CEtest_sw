...
if __name__ == '__main__':
    ...
    baseline    = ROOT.TH2F("Autocalibration baseline", "Autocalibration baseline", 16, 0, 16, 16, 0, 16)
    noise_width = ROOT.TH2F("Noise width", "Noise width", 16, 0, 16, 16, 0, 16)
    ROOT.gStyle.SetOptStat(0)
    scan_canvas.Divide(1, 2)
    for i in range(16):
        for j in range(16):
            etroc.wr_reg("disDataReadout", 0, row=i+1, col=j+1, broadcast=False)
            # current_offset = etroc.get_THoffset(row = i, col = j)
            threshold = etroc.auto_threshold_scan(i, j, offset="auto")
            print(i+1, j+1, threshold)
            baseline.SetBinContent(i+1, j+1, threshold[0])
            noise_width.SetBinContent(i+1, j+1, threshold[1])
    scan_canvas.cd(1)
