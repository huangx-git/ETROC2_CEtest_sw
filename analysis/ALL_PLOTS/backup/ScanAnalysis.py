...
def plots(data, x_axis):
    ...
    for d_i, d in enumerate(data):
        ...
        for p in [0,1,2,3]:
            ...
            print(len(toa_),len(LP2_20_6))
            toa_ = toa_[event_sel]
            tot_ = tot_[event_sel]
            cal_ = cal_[event_sel]
            element_sel = ((cal_ > 195) & (cal_ < 205)) # & (tot_ > 4) # & (tot_ < 4) # ((cal_ > 195) & (cal_ < 210) & (tot_ > 1.0) & (tot_ < 1.5))
            toa_ = toa_[element_sel]
            tot_ = tot_[element_sel]
            cal_ = cal_[element_sel]
            tot_ = fill_empty_arrays(tot_)[:,0] # np.reshape(tot_, len(tot_))
            cal_ = fill_empty_arrays(cal_)[:,0] # np.reshape(cal_, len(cal_))
            toa_ = fill_empty_arrays(toa_)[:,0] # np.reshape(toa_, len(toa_))
            # tot_ = tot_[:,0] # np.reshape(tot_, len(tot_))
            # cal_ = cal_[:,0] # np.reshape(cal_, len(cal_))
            # cuts["CAL cut"] = np.round(100 * sum(np.any(element_sel, axis = 1)) / len(element_sel), 2)
            # cuts["Clock != 0"] = np.round(100 * sum(event_sel) / sum(np.any(element_sel, axis = 1)), 2)
            # event_sel_lessthen_11 = (np.any(element_sel, axis = 1) & (LP2_20_6 < 11.0)) # & (LP2_20_6 < 11.0))
            # cuts["Clock < 11"] = np.round(100 * sum(event_sel_lessthen_11) / sum(np.any(element_sel, axis = 1)), 2)
            # event_sel_morethen_11 = (np.any(element_sel, axis = 1) & (LP2_20_6 >= 11.0)) # & (LP2_20_6 < 11.0))
            # cuts["Clock > 11"] = np.round(100 * sum(event_sel_morethen_11) / sum(np.any(element_sel, axis = 1)), 2)
            # event_sel_morethen_11 = (np.any(element_sel, axis = 1) & (LP2_20_6 >= 11.0)) # & (LP2_20_6 < 11.0))
            # print(cuts["CAL cut"], cuts["Clock != 0"], cuts["Clock < 11"], cuts["Clock > 11"])
            # print(LP2_20_6[(LP2_20_6 < 11) & (LP2_20_6 != 0)])
            print(f"Row: 15, Col: {p}")
            #event_sel = ((event_sel)&(toa_!=-1))
            maps = np.ones(len(toa_), dtype = bool) # (toa_!=-1)
        ...
        print(pd.DataFrame(cutflow_tables))
    ...
    for i in range(len(x_axis)): # tot vs Clock
        ...
        # for p in range(4):
        print(labels[19], labels[3])
        print(len(labels[19]), len(labels[3]))
    ...
    if do_Delta_T_Gaussian_fit:
        ...
        for p in range(4):
            ...
            for j in range(len(x_axis)):
                ...
                # minimum = min(var[p][j]) # if len(var[p][j]) != 0 else 10000 for i in range(len(x_axis))])
                # maximum = max(var[p][j]) # if len(var[p][j]) != 0 else 0     for i in range(len(x_axis))])
                if j != 3: continue
