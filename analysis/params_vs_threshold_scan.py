...
if __name__ == '__main__':
    ...
    # indices     = range(353, 364) # [252, 253, 254, 255, 256, 257, 258, 259]

    # filelist = os.listdir(f"../output/Threshold_scan_50/")
    # intensities = range(0, 55, 5) # [110, 111, 112, 113, 114, 115, 116, 117]
    ...
    for f_index, file in enumerate(indices):
        ...
        if len(filename) > 0:
            ...
            binary_filename = glob.glob(f"../output/output_run_{file}_time_*.dat")
            binary_filename = binary_filename[0].split('../output/')[1][:-4]
            print(binary_filename)
            # os.system("cd ../")
            os.system(f"python3 data_dumper.py --input {binary_filename}")
            # os.system("cd -")
        filename = glob.glob(f"../output/output_run_{file}_time_*.json")[0]
        ...
        for param in params:
            for pixel in pixels:
                ...
                if np.isnan(mean_variable):
                    print(filename)
                    # pdb.set_trace()
                    print(variable)
                    print(np.sum(variable))
                if param == "toa":
    ...
    ax[0].set_xlabel("Baseline offset values")
    ax[0].legend()
    # plt.legend()
    # plt.show()
    ...
    ax[1].set_xlabel("Baseline offset values")
    ax[1].legend()
    # plt.title("Calculation of mean TOA and TOT for different intensities.")
    plt.show()
    fig.savefig("param_vs_vth_70.png")
    # # CAL
    # for pixel in pixels:
